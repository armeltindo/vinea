  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Gouvernance des Départements</h2>
          <p className="text-sm text-slate-500 font-medium italic">Vinea : Structurez vos ministères et suivez vos projets.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={handleAnalyze} disabled={isAnalyzing} className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-xl text-xs font-medium disabled:opacity-50">
            <Sparkles size={16} /> {isAnalyzing ? 'Analyse...' : 'Analyse Stratégique'}
          </button>
          <button onClick={() => { setDeptFormData({ name: '', description: '', presidentId: members[0].id, memberIds: [], status: 'Actif', color: '#4f46e5' }); setEditingDept(null); setIsDeptFormOpen(true); }} className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-semibold hover:bg-indigo-700 transition-all shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-end justify-between">
            <span className="text-3xl font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{stats.totalDepts}</span>
        ...
        >
          <div className="flex items-end justify-between">
            <span className="text-3xl font-bold text-slate-900 group-hover:text-amber-600 transition-colors">{stats.totalActivities}</span>
            <span className="text-xs font-semibold text-indigo-600 bg-indigo-50 px-2 py-1 rounded uppercase">Total</span>
          </div>
        </Card>
        <Card 
          title="Réalisation" 
        ...
        >
          <div className="flex items-end justify-between">
            <span className="text-3xl font-bold text-emerald-600">{stats.realizationRate}%</span>
        ...
        >
          <div className="flex items-end justify-between">
            <span className="text-xl font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors">{formatCurrency(stats.totalCost)}</span>
          </div>
        </Card>
      </div>
              ...
              <div className="p-8 space-y-6">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <h3 className="text-xl font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors">{dept.name}</h3>
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">Président</span>
                    <span className="text-xs font-semibold text-slate-800">{getMemberName(dept.presidentId)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">Membres</span>
                    <div className="flex items-center gap-2">
                       <Users size={14} className="text-indigo-400" />
                       <span className="text-xs font-semibold text-slate-800">{dept.memberIds.length} fidèles</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">Activités Réalisées</span>
                ...
                <button onClick={() => { setActivityFormData({ deptId: dept.id, status: ActivityStatus.PLANIFIEE }); setIsActivityFormOpen(true); }} className="w-full py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-medium text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all flex items-center justify-center gap-2">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="px-6 py-4 text-xs font-medium text-slate-500">Activité</th>
                  <th className="px-6 py-4 text-xs font-medium text-slate-500">Département</th>
                  <th className="px-6 py-4 text-xs font-medium text-slate-500">Responsable</th>
                  <th className="px-6 py-4 text-xs font-medium text-slate-500">Coût</th>
                  <th className="px-6 py-4 text-xs font-medium text-slate-500">Délai</th>
                  <th className="px-6 py-4 text-xs font-medium text-slate-500">Statut</th>
                  ...
                  return (
                    <tr key={activity.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        {activity.observations && <p className="text-xs text-slate-400 italic line-clamp-1">{activity.observations}</p>}
                      </td>
                      <td className="px-6 py-4">
                        <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded text-xs font-medium" style={{ borderLeft: `3px solid ${dept?.color || '#ccc'}` }}>
                          {dept?.name || 'Inconnu'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <p className="text-xs font-semibold text-slate-900">{formatCurrency(activity.cost || 0)}</p>
                      </td>
                      <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
            <div className="bg-indigo-600 p-8 text-white">
               <h3 className="text-xl font-semibold">{editingDept ? 'Modifier Département' : 'Nouveau Département'}</h3>
               <p className="text-xs text-indigo-200 mt-1">Structure organisationnelle</p>
            </div>
            <form onSubmit={saveDept} className="p-8 space-y-6 bg-slate-50/30">
               <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Description / Mission</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Président (Responsable)</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Couleur d'identification</label>
               ...
               <div className="flex gap-3">
                  <button type="button" onClick={() => setIsDeptFormOpen(false)} className="flex-1 py-3.5 bg-white border border-slate-200 text-slate-500 rounded-2xl text-sm font-medium">Annuler</button>
                  <button type="submit" className="flex-1 py-3.5 bg-indigo-600 text-white rounded-2xl text-sm font-semibold shadow-xl shadow-indigo-100 flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="bg-amber-500 p-8 text-white shrink-0">
               <h3 className="text-xl font-semibold">{editingActivity ? 'Modifier l\'Activité' : 'Planifier une Activité'}</h3>
               <p className="text-xs font-bold text-amber-100 mt-1">Registre des actions</p>
            </div>
            <form onSubmit={saveActivity} className="p-8 space-y-6 bg-slate-50/30 overflow-y-auto custom-scrollbar">
               <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Intitulé de l'activité *</label>
                    <input type="text" required value={activityFormData.title} onChange={(e) => setActivityFormData({...activityFormData, title: e.target.value})} className="w-full px-5 py-3.5 bg-white border border-slate-200 rounded-2xl outline-none text-sm font-semibold" placeholder="Ex: Répétition générale" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-slate-500 ml-1">Personne responsable *</label>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-slate-500 ml-1">Personne associée</label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-slate-500 ml-1">Coût d'exécution (FCFA)</label>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-slate-500 ml-1">Délai d'exécution</label>
               ...
               <div className="flex gap-3 pt-4 shrink-0">
                  <button type="button" onClick={() => setIsActivityFormOpen(false)} className="flex-1 py-3.5 bg-white border border-slate-200 text-slate-500 rounded-2xl text-sm font-medium">Annuler</button>
                  <button type="submit" className="flex-1 py-4 bg-amber-500 text-white rounded-2xl text-xs font-medium shadow-xl shadow-amber-100 flex items-center justify-center gap-2 hover:bg-amber-600 transition-all">
