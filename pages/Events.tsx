    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card title="Événements" subtitle="Année en cours" icon={<Calendar size={20} className="text-indigo-600" />}>
          <div className="flex items-end justify-between">
            <span className="text-3xl font-semibold">{events.length}</span>
        ...
        <Card title="Inscriptions" subtitle="Remplissage global" icon={<Users size={20} className="text-indigo-600" />}>
          <div className="flex items-end justify-between">
            <span className="text-3xl font-semibold">{events.reduce((sum, e) => sum + e.registered, 0)}</span>
            <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-2 py-1 rounded">Cumul</span>
          </div>
        </Card>
        <Card title="Engagement Budget" subtitle="Total alloué" icon={<DollarSign size={20} className="text-emerald-600" />}>
          <div className="flex items-end justify-between">
            <span className="text-xl font-semibold truncate">{formatCurrency(events.reduce((sum, e) => sum + e.budget, 0))}</span>
          </div>
        </Card>
        <Card title="Impact" subtitle="Objectifs annuels" icon={<Target size={20} className="text-amber-500" />}>
          <div className="flex items-end justify-between">
            <span className="text-3xl font-semibold">{goals.length > 0 ? `${Math.round(goals.reduce((a, b) => a + b.value, 0) / goals.length)}%` : '--%'}</span>
            ...
            <div className="flex gap-2">
               <div className="relative group">
                ...
                <input 
                  ...
                  className="pl-9 pr-3 py-2 text-xs font-medium border border-slate-200 rounded-xl outline-none focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-200 bg-white shadow-sm w-40 md:w-60" 
                />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            {filteredEvents.length > 0 ? filteredEvents.map((event) => (
              <div 
                ...
                className="bg-white border border-slate-200 rounded-2xl overflow-hidden flex flex-col md:flex-row hover:border-indigo-300 transition-all group shadow-sm cursor-pointer"
              >
                <div className="md:w-64 h-48 md:h-auto overflow-hidden relative bg-slate-100">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1 bg-white/90 backdrop-blur-md rounded-full text-xs font-semibold text-indigo-600 shadow-sm border border-white/50">
                      {event.category}
                    </span>
                  </div>
                </div>
                <div className="flex-1 p-8 space-y-4">
                  <div className="flex justify-between items-start gap-4">
                    <div className="min-w-0">
                      <h4 className="text-xl font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors leading-tight truncate">{event.title}</h4>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <div className="space-y-1">
                      <p className="text-xs font-medium text-slate-500">Période</p>
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-medium text-slate-500">Inscrits</p>
                      <p className="text-xs font-medium text-slate-700 flex items-center gap-1.5">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs font-semibold text-emerald-600">{formatCurrency(event.budget).split(' ')[0]} F</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs font-medium text-slate-500">Remplissage</p>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs font-semibold text-slate-500">{Math.round((event.registered/(event.target || 1))*100)}%</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-6 border-t border-slate-50 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button 
                        ...
                        className="px-5 py-2 text-xs font-semibold text-indigo-600 border border-indigo-100 rounded-xl hover:bg-indigo-50 transition-all"
                      >
                        Gérer Budget
                      </button>
                      <button 
                        onClick={(e) => { e.stopPropagation(); setSelectedEvent(event); }}
                        className="flex items-center gap-1.5 px-5 py-2 text-xs font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100"
                      >
        ...
        <div className="lg:w-80 space-y-6">
          <Card title="Objectifs stratégiques" icon={<Target size={18} />}>
            <div className="space-y-6">
              {goals.length > 0 ? goals.map((obj) => (
                ...
                <p className="text-xs text-slate-300 italic text-center py-4">Aucun objectif défini</p>
              )}
            </div>
            <button 
              onClick={() => setIsGoalsModalOpen(true)}
              className="w-full mt-8 py-3 text-xs font-semibold text-indigo-600 bg-indigo-50/50 border border-indigo-100 rounded-xl hover:bg-indigo-100 transition-all flex items-center justify-center gap-2"
            >
          ...
          <Card title="Départements Mobiles" icon={<Tag size={18} />}>
            ...
            <button 
              ...
              className="w-full mt-6 flex items-center justify-center gap-2 py-4 text-xs font-semibold text-indigo-600 bg-indigo-50 rounded-2xl hover:bg-indigo-100 transition-all border border-indigo-100 shadow-sm disabled:opacity-50"
            >
      ...
      {isGoalsModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100 flex flex-col max-h-[90vh]">
            <div className="bg-slate-900 p-8 text-white relative shrink-0">
               ...
               <h3 className="text-xl font-semibold">Objectifs Stratégiques</h3>
               <p className="text-xs font-bold text-white/50 mt-1">Planification annuelle</p>
            </div>
            <div className="p-8 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
               <form onSubmit={handleAddGoal} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-3">
                  <p className="text-xs font-medium text-slate-500">Ajouter un nouvel objectif</p>
                  <input 
                    type="text" required
                  ...
                  <button type="submit" className="w-full py-2 bg-indigo-600 text-white rounded-xl text-xs font-medium">
                    Ajouter à la liste
                  </button>
               </form>
                 ...
                 {goals.map(goal => (
                   <div key={goal.id} className="space-y-3 p-4 border border-slate-100 rounded-2xl relative group">
                      ...
                      <div className="flex justify-between items-center px-1">
                         <label className="text-xs font-medium text-slate-500">{goal.label}</label>
                         <span className="text-xs font-semibold text-indigo-600">{goal.value}%</span>
                      </div>
                      <input 
                        type="range" 
                        min="0" max="100" 
            ...
            <div className="p-8 border-t border-slate-50 bg-slate-50 shrink-0">
               <button 
                ...
                className="w-full py-4 bg-slate-900 text-white rounded-2xl text-xs font-medium shadow-xl"
               >
                 Terminer la gestion
               </button>
            </div>
      ...
      {isAssignmentsModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100 flex flex-col">
            <div className="bg-indigo-600 p-8 text-white relative">
               ...
               <h3 className="text-xl font-semibold">Affectation d'Équipes</h3>
               <p className="text-xs font-bold text-white/70 mt-1">Coordination logistique</p>
            </div>
            
            <form onSubmit={handleAddAssignment} className="p-8 space-y-6">
               <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Département</label>
                    <select 
                      value={newAssignment.dept}
                      onChange={(e) => setNewAssignment({...newAssignment, dept: e.target.value as any})}
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none text-sm font-normal tracking-wide focus:ring-4 focus:ring-indigo-500/5 transition-all"
                    >
                      {availableDepartments.map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Événement cible</label>
                    <select 
                      required
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Statut de préparation</label>
                    <select 
               ...
               <button 
                  ...
                  className="w-full py-3.5 bg-indigo-600 text-white rounded-2xl text-sm font-semibold shadow-xl shadow-indigo-100 flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all disabled:opacity-50"
               >
            ...
            <div className="px-8 pb-8 flex flex-col gap-2">
              <h4 className="text-xs font-medium text-slate-500 mb-1">Affectations récentes</h4>
              <div className="max-h-[150px] overflow-y-auto custom-scrollbar space-y-2 pr-1">
                 {assignments.map(a => (
                   <div key={a.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                      <span className="text-xs font-medium text-slate-700 truncate max-w-[150px]">{a.dept}</span>
                      <div className="flex items-center gap-2">
                         <span className="text-xs font-medium text-indigo-600">{a.status}</span>
                         <button onClick={() => handleDeleteAssignment(a.id)} className="text-slate-300 hover:text-rose-500"><X size={12} /></button>
                      </div>
                   </div>
                 ))}
                 {assignments.length === 0 && <p className="text-xs text-slate-300 italic text-center py-2">Aucune équipe affectée</p>}
              </div>
            </div>
          </div>
        </div>
      ...
      {isBudgetModalOpen && selectedEvent && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100 flex flex-col max-h-[85vh]">
            <div className="bg-emerald-600 p-8 text-white relative shrink-0">
               ...
               <h3 className="text-xl font-semibold">Gestion Budgétaire</h3>
               <p className="text-xs font-bold text-white/80 mt-1">{selectedEvent.title}</p>
            </div>
            
            <div className="p-8 space-y-8 flex-1 overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-3 gap-4">
                 <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                    <p className="text-xs font-medium text-slate-500 mb-1">Budget Alloué</p>
                    <p className="text-sm font-semibold text-slate-900">{formatCurrency(selectedEvent.budget)}</p>
                 </div>
                 <div className="bg-rose-50 p-4 rounded-2xl border border-rose-100 text-center text-rose-600">
                    <p className="text-xs font-semibold text-rose-400 mb-1">Total Dépensé</p>
                    <p className="text-sm font-semibold">{formatCurrency(selectedEvent.expenses)}</p>
                 </div>
                 <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 text-center text-emerald-600">
                    <p className="text-xs font-semibold text-emerald-400 mb-1">Solde Disponible</p>
                    <p className="text-sm font-semibold">{formatCurrency(selectedEvent.budget - selectedEvent.expenses)}</p>
                 </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-semibold text-indigo-600">{selectedEvent.expenseList?.length || 0} transaction(s)</span>
                </div>
                  ...
                  {selectedEvent.expenseList?.map(expense => (
                    <div key={expense.id} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl group hover:border-rose-100 hover:bg-rose-50/20 transition-all">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-xs font-semibold text-slate-800">{expense.label}</p>
                            <p className="text-xs text-slate-400">{new Date(expense.date).toLocaleDateString('fr-FR')}</p>
                          </div>
                       </div>
                       <div className="flex items-center gap-4">
                          <span className="text-sm font-semibold text-rose-600">-{formatCurrency(expense.amount)}</span>
                          <button 
              ...
              <form onSubmit={handleAddExpense} className="p-6 bg-slate-50 border border-slate-100 rounded-xl space-y-4">
                 <h5 className="text-xs font-semibold text-slate-600 mb-2">Ajouter une dépense</h5>
                 <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="relative">
                      ...
                      />
                      <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-300">FCFA</span>
                    </div>
                 </div>
                 <button 
                  type="submit"
                  className="w-full py-3 bg-emerald-600 text-white rounded-xl text-xs font-medium shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 hover:bg-emerald-700 transition-all"
                 >
          ...
          <div className="relative w-full max-w-2xl bg-white shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col rounded-2xl overflow-hidden max-h-[90vh]">
            <div className="relative h-64 shrink-0 bg-slate-200">
              ...
              <div className="absolute bottom-8 left-10 right-10">
                 <span className="px-3 py-1 bg-indigo-600 text-white rounded-full text-xs font-medium mb-3 inline-block">
            ...
            <div className="flex-1 overflow-y-auto p-10 custom-scrollbar bg-slate-50/30 space-y-8">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex items-center gap-4">
                   ...
                   <div>
                      <p className="text-xs font-medium text-slate-500 mb-1">Lieu</p>
                      <p className="text-sm font-semibold text-slate-800 leading-none">{selectedEvent.location}</p>
                   </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div 
                  ...
                  className="bg-indigo-600 p-8 rounded-2xl text-white shadow-xl shadow-indigo-100 cursor-pointer group hover:scale-[1.02] transition-transform"
                >
                   <div className="flex justify-between items-center mb-6">
                     <h4 className="text-xs font-medium text-indigo-200">Budget engagé</h4>
                     <DollarSign size={20} className="text-indigo-200 group-hover:rotate-12 transition-transform" />
                   </div>
                   <p className="text-3xl font-semibold">{formatCurrency(selectedEvent.budget)}</p>
                ...
                <div className="bg-emerald-600 p-8 rounded-2xl text-white shadow-xl shadow-emerald-100">
                   <div className="flex justify-between items-center mb-6">
                     <h4 className="text-xs font-medium text-emerald-200">Remplissage</h4>
                     <Users size={20} className="text-emerald-200" />
                   </div>
                   <p className="text-3xl font-semibold">{selectedEvent.registered} / {selectedEvent.target}</p>
                   <div className="mt-6 space-y-2">
                      <div className="flex justify-between text-xs font-medium">
        ...
        <div className="fixed inset-0 z-[170] overflow-hidden flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="bg-indigo-600 p-8 text-white shrink-0">
              ...
              <p className="text-xs text-indigo-200">Planification Vinea</p>
            </div>
            
            <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto p-10 space-y-6 custom-scrollbar bg-slate-50/30">
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-500 ml-1">Titre de l'événement</label>
                  <input 
                    type="text" required autoFocus
                    placeholder="Ex: Convention Impact 2024"
                    value={formData.title}
                    onChange={(e) => setFormData({...formData, title: e.target.value})}
                    className="w-full px-5 py-3.5 bg-white border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-300 outline-none text-sm font-semibold shadow-sm transition-all"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Date fin</label>
                    <input 
                      type="date" required
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-500 ml-1">Localisation</label>
                  <input 
                    type="text" required
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-500 ml-1">Budget prévisionnel (FCFA)</label>
                  <input 
                    type="number"
                    placeholder="0"
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-slate-500 ml-1">Description / Notes</label>
                  <textarea 
                    rows={4}
              ...
              <div className="pt-8 flex gap-4 pb-4">
                <button 
                  ...
                  className="flex-1 py-3.5 bg-white border border-slate-200 text-slate-500 rounded-2xl text-sm font-medium hover:bg-slate-50 transition-all shadow-sm"
                >
                  Annuler
                </button>
                <button 
                  ...
                  className="flex-1 py-3.5 bg-indigo-600 text-white rounded-2xl text-sm font-semibold hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50"
                >
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl p-10 text-center animate-in zoom-in-95 duration-200 border border-slate-100">
            ...
            <div className="flex flex-col gap-3 mt-8">
              <button onClick={confirmDelete} className="w-full py-3.5 bg-rose-600 text-white rounded-2xl text-sm font-semibold hover:bg-rose-700 transition-all shadow-xl shadow-rose-200">Confirmer suppression</button>
              <button onClick={() => setIsDeleteConfirmOpen(false)} className="w-full py-3.5 bg-slate-50 text-slate-500 rounded-2xl text-sm font-medium hover:bg-slate-100 transition-all border border-slate-200">Annuler</button>
            </div>
          </div>
        </div>
      )}
