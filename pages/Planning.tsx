  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Planning Stratégique</h2>
        ...
        <div className="flex gap-2">
          <button onClick={handleAnalyze} disabled={isAnalyzing} className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-xl text-sm font-semibold hover:bg-indigo-100 transition-all hover:bg-indigo-100 disabled:opacity-50">
        ...
        <div className="flex bg-white p-1 rounded-2xl border border-slate-200 shadow-sm w-fit overflow-x-auto max-w-full no-scrollbar">
          {availableYears.map(year => (
            <button 
              ...
              className={cn(
                "px-6 py-2 rounded-xl text-xs font-medium transition-all shrink-0", 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card title="Départements" icon={<Briefcase size={20} className="text-indigo-600" />} className="cursor-pointer active:scale-95" onClick={() => setActiveView('depts')}>
          <div className="flex items-end justify-between"><span className="text-3xl font-bold text-slate-900">{stats.totalDepts}</span></div>
        </Card>
        <Card title="Planification" icon={<Target size={20} className="text-amber-500" />} className="cursor-pointer active:scale-95" onClick={() => setActiveView('planning')}>
          <div className="flex items-end justify-between"><span className="text-3xl font-bold text-slate-900">{stats.totalActivities}</span></div>
        </Card>
        <Card title="Réalisation" icon={<CheckCircle2 size={20} className="text-emerald-500" />}>
          <div className="flex items-end justify-between"><span className="text-3xl font-bold text-emerald-600">{stats.realizationRate}%</span><TrendingUp size={20} className="text-emerald-500 mb-1" /></div>
        </Card>
        <Card title="Coût Global" icon={<DollarSign size={20} className="text-slate-600" />}>
          <div className="flex items-end justify-between"><span className="text-xl font-semibold">{formatCurrency(stats.totalCost).replace(' F CFA', '')}</span></div>
        </Card>
      </div>
      ...
      {activeView === 'depts' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                <div className="p-8 space-y-6">
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-xl font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors truncate">{dept.name}</h3>
                      <p className="text-xs text-slate-400 font-bold mt-1">Département {churchName}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center overflow-hidden shrink-0 text-xs font-semibold text-indigo-600 uppercase">
                          {president?.photoUrl ? (
                        ...
                        <p className="text-xs font-semibold text-slate-800 truncate">{getMemberNameFormatted(dept.presidentId)}</p>
                      </div>
                    </div>
                  ...
                  <div className="pt-4" onClick={e => e.stopPropagation()}>
                    <button 
                      ...
                      className="w-full flex items-center justify-center gap-2 py-3 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-2xl text-xs font-medium hover:bg-indigo-600 hover:text-white transition-all shadow-sm"
                    >
          ...
          <div className="p-8 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center text-center opacity-40">
             <Settings2 size={32} className="text-slate-300 mb-2" />
             <p className="text-xs font-medium text-slate-500">Géré via les paramètres</p>
          </div>
        </div>
      ) : (
        <Card className="p-0 overflow-hidden border-slate-200 shadow-xl rounded-2xl animate-in slide-in-from-bottom-4 duration-500">
             ...
             <div className="flex flex-wrap gap-3 shrink-0">
                <div className="relative group">
                  ...
                  <select 
                    ...
                    className="pl-9 pr-8 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-normal outline-none focus:border-indigo-300 shadow-sm cursor-pointer hover:bg-slate-50 transition-all appearance-none"
                  >
                ...
                <button 
                  ...
                  className="px-6 py-3 bg-indigo-600 text-white rounded-2xl text-xs font-medium shadow-xl hover:bg-indigo-700 transition-all active:scale-95 flex items-center gap-2"
                >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-100">
                  <th className="px-10 py-5 text-xs font-medium text-slate-500 cursor-pointer hover:text-indigo-600 transition-colors" onClick={() => handleSort('title')}>
                  ...
                  <th className="px-10 py-5 text-xs font-medium text-slate-500">Porteur de Projet</th>
                  <th className="px-10 py-5 text-xs font-medium text-slate-500 cursor-pointer hover:text-indigo-600 transition-colors text-right" onClick={() => handleSort('cost')}>
                  ...
                  <th className="px-10 py-5 text-xs font-medium text-slate-500 cursor-pointer hover:text-indigo-600 transition-colors" onClick={() => handleSort('deadline')}>
                  ...
                  <th className="px-10 py-5 text-xs font-medium text-slate-500 text-right">Statut</th>
                  <th className="px-10 py-5 text-xs font-medium text-slate-500 text-right">Actions</th>
                </tr>
              </thead>
                    ...
                    >
                      <td className="px-10 py-6">
                        <div className="flex items-center gap-4">
                           ...
                           <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                <p className="text-sm font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors truncate max-w-[200px]">{activity.title}</p>
                                {isRecurring && (
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                 <span className="text-xs font-medium text-slate-500">{dept?.name || 'Général'}</span>
                              </div>
                           </div>
                        </div>
                      </td>
                      <td className="px-10 py-6">
                        <div className="flex items-center gap-3">
                           <div className="w-9 h-9 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center text-xs font-medium text-slate-500 shadow-sm group-hover:scale-105 transition-transform overflow-hidden">
                              {respMember?.photoUrl ? (
                      ...
                      <td className="px-10 py-6 text-right font-semibold text-slate-900">{(activity.cost || 0).toLocaleString('fr-FR')}</td>
                      <td className="px-10 py-6">
                           ...
                           {activity.deadline ? (
                             <>
                               <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs font-medium text-slate-700">{new Date(activity.deadline).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })}</span>
                               </div>
                      ...
                      <td className="px-10 py-6 text-right">
                        <span className={cn(
                          "inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium shadow-sm border",
          ...
          <div className="relative w-full max-w-lg bg-white shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col rounded-2xl overflow-hidden max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-8 bg-slate-50/30">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex flex-col items-center text-center space-y-2">
                   ...
                   <div>
                      ...
                      <p className="text-xs font-semibold text-slate-800 leading-none mt-1">{getMemberNameFormatted(selectedActivityForDetails.responsibleId)}</p>
                   </div>
                </div>
                <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex flex-col items-center text-center space-y-2">
                   ...
                   <div>
                      <p className="text-xs font-medium text-slate-500">Budget estimé</p>
                      <p className="text-xs font-semibold text-slate-800 leading-none mt-1">{formatCurrency(selectedActivityForDetails.cost || 0)}</p>
                   </div>
                </div>
                <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex flex-col items-center text-center space-y-2">
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm flex flex-col items-center text-center space-y-2">
                   ...
                   <div>
                      <p className="text-xs font-medium text-slate-500">Récurrence</p>
                      <p className="text-xs font-semibold text-slate-800 leading-none mt-1">{selectedActivityForDetails.recurrence || 'Ponctuelle'}</p>
                   </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                 <h4 className="text-xs font-medium text-slate-500">État de l'activité</h4>
                 <div className={cn(
                   ...
                   selectedActivityForDetails.status === ActivityStatus.REALISEE ? "bg-emerald-50 border-emerald-100" : "bg-white border-slate-100"
                    ...
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-sm font-medium text-slate-700">{selectedActivityForDetails.status}</span>
                    </div>
                    {selectedActivityForDetails.lastRealizedAt && (
                      <span className="text-xs text-slate-400 italic">Dernière fois : {new Date(selectedActivityForDetails.lastRealizedAt).toLocaleDateString()}</span>
                    )}
                 </div>
              </div>
            </div>
            <div className="p-10 border-t border-slate-100 bg-white flex flex-col gap-3 shrink-0">
              {selectedActivityForDetails.status !== ActivityStatus.REALISEE && (
                <button 
                  ...
                  className="w-full py-4 bg-emerald-600 text-white rounded-2xl text-xs font-medium shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all flex items-center justify-center gap-2"
                >
        ...
        <div className="fixed inset-0 z-[190] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-indigo-600 p-8 text-white shrink-0">
               <h3 className="text-xl font-semibold">{editingDept ? 'Modifier Département' : 'Configuration Département'}</h3>
            ...
            <form onSubmit={saveDept} className="p-8 space-y-6 bg-slate-50/30 overflow-y-auto custom-scrollbar">
               <div className="space-y-4">
                  <div className="space-y-1.5"><label className="text-xs font-medium text-slate-500 ml-1">Mission / Vision</label><textarea rows={3} value={deptFormData.description || ''} onChange={e => setDeptFormData({...deptFormData, description: e.target.value})} className="w-full px-5 py-4 bg-white border border-slate-200 rounded-2xl outline-none text-sm font-medium resize-none shadow-sm" placeholder="Objectifs du département..." /></div>
                  <div className="space-y-1.5 relative">
                    <label className="text-xs font-medium text-slate-500 ml-1 flex items-center gap-2"><UserCheck size={12} className="text-indigo-600" /> Responsable Principal</label>
                    <div className="relative group">
                    ...
                    {presidentSearch && !deptFormData.presidentId && (
                      <div className="absolute z-30 left-0 right-0 top-full mt-1 max-h-40 overflow-y-auto bg-white border border-slate-200 rounded-2xl shadow-xl custom-scrollbar">
                        {members.filter(m => `${formatFirstName(m.firstName)} ${m.lastName}`.toLowerCase().includes(presidentSearch.toLowerCase())).map(m => (
                          <button key={m.id} type="button" onClick={() => { setDeptFormData({...deptFormData, presidentId: m.id}); setPresidentSearch(`${formatFirstName(m.firstName)} ${m.lastName.toUpperCase()}`); }} className="w-full text-left px-5 py-4 text-xs font-medium hover:bg-indigo-50 flex items-center gap-4 transition-colors border-b border-slate-50 last:border-0 flex items-center gap-3">
               ...
               <div className="flex gap-3 pt-8 pb-4">
                  ...
                  <button type="submit" disabled={isSubmitting} className="flex-[2] py-3.5 bg-indigo-600 text-white rounded-2xl text-sm font-semibold shadow-xl flex items-center justify-center gap-2 disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            <div className="bg-amber-500 p-8 text-white shrink-0">
               <h3 className="text-xl font-semibold">{editingActivity ? 'Modifier' : 'Planifier'}</h3>
            ...
            <form onSubmit={handleSaveActivity} className="p-8 space-y-8 bg-slate-50/30 overflow-y-auto custom-scrollbar">
               <div className="space-y-6">
                  <div className="space-y-1.5"><label className="text-xs font-medium text-slate-500 ml-1">Activité *</label><input type="text" required value={activityFormData.title} onChange={e => setActivityFormData({...activityFormData, title: e.target.value})} className="w-full px-5 py-4 bg-white border border-slate-200 rounded-2xl outline-none text-sm font-semibold shadow-sm" placeholder="Intitulé de l'action..." /></div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5"><label className="text-xs font-medium text-slate-500 ml-1">Département *</label><select required value={activityFormData.deptId} onChange={e => setActivityFormData({...activityFormData, deptId: e.target.value})} className="w-full px-4 py-4 bg-white border border-slate-200 rounded-2xl outline-none text-sm font-normal shadow-sm">{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
                    <div className="space-y-1.5"><label className="text-xs font-medium text-slate-500 ml-1">Statut</label><select required value={activityFormData.status} onChange={e => setActivityFormData({...activityFormData, status: e.target.value as ActivityStatus})} className="w-full px-4 py-4 bg-white border border-slate-200 rounded-2xl outline-none text-sm font-normal shadow-sm">{Object.values(ActivityStatus).map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                  </div>
                  <div className="space-y-1.5 relative">
                    ...
                    {responsibleSearch && !activityFormData.responsibleId && (
                      <div className="absolute z-30 left-0 right-0 top-full mt-1 max-h-40 overflow-y-auto bg-white border border-slate-200 rounded-2xl shadow-2xl">
                        {members.filter(m => `${formatFirstName(m.firstName)} ${m.lastName}`.toLowerCase().includes(responsibleSearch.toLowerCase())).map(m => (
                          <button key={m.id} type="button" onClick={() => { setActivityFormData({...activityFormData, responsibleId: m.id}); setResponsibleSearch(`${formatFirstName(m.firstName)} ${m.lastName.toUpperCase()}`); }} className="w-full text-left px-4 py-3 text-xs font-medium hover:bg-amber-50 flex items-center gap-4 transition-colors border-b border-slate-50 last:border-0 flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-white border border-slate-100 flex items-center justify-center overflow-hidden shrink-0 text-xs font-medium text-slate-500">
                               {m.photoUrl ? (
                                 <img src={m.photoUrl} alt="" className="w-full h-full object-cover" />
                               ) : (
                                 getInitials(m.firstName, m.lastName)
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5"><label className="text-xs font-medium text-slate-500 ml-1">{activityFormData.recurrence !== 'Ponctuelle' ? 'Date de mise en vigueur' : 'Échéance'}</label><input type="date" value={activityFormData.deadline || ''} onChange={(e) => setActivityFormData({...activityFormData, deadline: e.target.value})} className="w-full px-5 py-4 bg-white border border-slate-200 rounded-2xl outline-none text-xs font-bold shadow-sm" /></div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-slate-500 ml-1">Récurrence</label>
                      <select 
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Coût par cycle (FCFA)</label>
                    <input 
                      type="number" 
                      value={activityFormData.cost ?? 0}
                      onChange={(e) => setActivityFormData({...activityFormData, cost: parseInt(e.target.value) || 0})}
                      className="w-full px-5 py-4 bg-white border border-slate-200 rounded-2xl outline-none text-sm font-semibold text-indigo-600 shadow-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" 
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-slate-500 ml-1">Observations / Commentaires</label>
               ...
               <div className="flex gap-3 pt-6 pb-2">
                <button type="button" onClick={() => setIsActivityFormOpen(false)} disabled={isSubmitting} className="flex-1 py-4 bg-white border border-slate-200 text-slate-500 rounded-2xl text-xs font-medium">Annuler</button>
                <button type="submit" disabled={isSubmitting} className="flex-[2] py-4 bg-amber-500 text-white rounded-2xl text-xs font-medium shadow-xl flex items-center justify-center gap-2 hover:bg-amber-600 transition-all disabled:opacity-50">
          ...
          <div className="relative w-full max-w-lg bg-white shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col rounded-2xl overflow-hidden max-h-[90vh]">
                    ...
                    <>
                      ...
                      {isDeptDescExpanded && (
                        <button 
                          ...
                          className="text-indigo-200 hover:text-white ml-2 text-xs font-medium align-middle"
                        >
                          [Réduire]
                        </button>
                      )}
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-8 bg-slate-50/30">
              <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center text-center space-y-3">
                 <div className="w-20 h-20 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 font-semibold text-2xl uppercase overflow-hidden shadow-inner">
                 ...
                 <div>
                    <p className="text-xs font-medium text-slate-500">Responsable Principal</p>
                  ...
                  {selectedDeptForDetails.memberIds.map(mId => {
                    ...
                    return (
                      <div key={mId} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl group hover:border-indigo-200 transition-all">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-xs font-medium text-slate-400 overflow-hidden shrink-0 text-xs">
                             {m?.photoUrl ? (
                               <img src={m.photoUrl} alt="" className="w-full h-full object-cover" />
                             ) : (
                               getInitials(m?.firstName, m?.lastName)
                          ...
                          <span className="text-xs font-semibold text-slate-800">{m ? `${formatFirstName(m.firstName)} ${m.lastName.toUpperCase()}` : '??'}</span>
                        </div>
                        {m?.type && <span className="text-xs font-semibold text-indigo-400 bg-indigo-50/50 px-2 py-0.5 rounded-lg uppercase">{m.type}</span>}
                      </div>
                    );
                  })}
                </div>
      ...
      {isDeleteDeptConfirmOpen && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl p-10 text-center border border-slate-100 animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-2xl font-semibold text-slate-900">Supprimer Département ?</h3>
            ...
            <div className="flex flex-col gap-3 mt-8">
              <button onClick={confirmDeleteDept} className="w-full py-4 bg-rose-600 text-white rounded-2xl text-xs font-medium shadow-xl shadow-rose-200 hover:bg-rose-700 transition-all active:scale-95">Confirmer</button>
              <button onClick={() => setIsDeleteDeptConfirmOpen(false)} className="w-full py-4 bg-slate-50 text-slate-600 rounded-2xl text-xs font-medium border border-slate-200 hide-scrollbar overflow-y-auto">Annuler</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isDeleteActivityConfirmOpen && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl p-10 text-center animate-in zoom-in-95 duration-200">
            <div className="w-20 h-20 bg-rose-50 text-rose-600 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-inner border border-rose-100/50">
              <AlertTriangle size={40} />
            </div>
            <h3 className="text-2xl font-semibold text-slate-900">Supprimer l'Activité ?</h3>
            <div className="flex flex-col gap-3 mt-8">
              <button onClick={confirmDeleteActivity} className="w-full py-4 bg-rose-600 text-white rounded-2xl text-xs font-medium shadow-xl shadow-rose-200 hover:bg-rose-700 transition-all active:scale-95">Confirmer</button>
              <button onClick={() => setIsDeleteActivityConfirmOpen(false)} className="w-full py-4 bg-slate-50 text-slate-600 rounded-2xl text-xs font-medium border border-slate-200 hover:bg-slate-100 transition-all">Annuler</button>
            </div>
          </div>
        </div>
      )}
