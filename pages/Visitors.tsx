                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          {visitor.parrainId ? (() => { const parrain = members.find(m => m.id === visitor.parrainId); return parrain ? <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 flex items-center gap-1"><ShieldCheck size={10}/> {formatFirstName(parrain.firstName)} {parrain.lastName.toUpperCase()}</span> : <span className="text-xs text-slate-400 font-bold">{visitor.source}</span>; })() : <span className="text-xs text-slate-400 font-bold">Non suivi</span>}
                        </div>
                      </div>
                    </div>
                  </td>
